...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            self.main_frame, text="SECURE CONNECTION CLIENT",
        ...
        self.log("SYS: BekreVPN Client v1.3")
    ...
    def _check_ip_t(self):
        try:
            ...
            if r.returncode == 0:
                ...
                self.after(0, lambda: self.log("NET: Public IP = " + ip))
        except:
    ...
    def _connect_wg_click(self):
        ...
        self.log("INIT: WireGuard protocol selected")
    ...
    def _connect_wg(self):
        ...
        if self.is_linux:
            ...
            if r and r.returncode == 0:
                # Gercek handshake dogrulamasi
                time.sleep(2)
                hs = self._cmd(["wg", "show", "wg0", "latest-handshakes"])
                if hs and hs.stdout.strip():
                    ts = hs.stdout.strip().split()[-1]
                    try:
                        ago = int(time.time()) - int(ts)
                        self.after(0, lambda a=ago: self.log(
                            "WG: Handshake OK (" + str(a) + "s ago)" if a < 30
                            else "WG: No handshake yet, check server"))
                    except:
                        pass
                self.after(0, lambda: self.log("SUCCESS: WireGuard tunnel UP"))
                self.after(0, lambda: self._set_status(True, "wg"))
                self.after(1000, self._check_ip)
            else:
                e = r.stderr[:80] if r and r.stderr else "wg-quick failed"
    ...
    def _connect_ovpn_click(self):
        ...
        self.status_label.configure(text="CONNECTING...")
        self.status_dot.configure(text_color="#ffcc00")
        self.log("INIT: OpenVPN Stealth mode (TCP 443)")
    ...
    def _connect_ovpn(self):
        ...
        # Stunnel baslat
        self.after(0, lambda: self.log("EXEC: stunnel starting..."))
        ...
        try:
            ...
            self.after(0, lambda: self.log("ERR: stunnel bulunamadi!"))
            self.after(0, lambda: self._set_status(False))
            return
        time.sleep(2)
        self.after(0, lambda: self.log("EXEC: stunnel OK -> 127.0.0.1:1194"))

        # OpenVPN baslat — stdout gercek zamanlı okunuyor
        self.after(0, lambda: self.log("EXEC: OpenVPN starting..."))
        ...
        try:
            ...
            self.after(0, lambda: self.log("ERR: openvpn bulunamadi!"))
            self.after(0, lambda: self._set_status(False))
            return
    ...
    def _monitor_ovpn(self):
        """OpenVPN stdout'u gercek zamanli oku ve GUI'ye yaz."""
        ...
        for line in iter(self.ovpn_process.stdout.readline, b''):
            ...
            if any(k in d for k in [
                ...
                short = d[-90:] if len(d) > 90 else d
                self.after(0, lambda s=short: self.log("OVPN: " + s))

            if "Initialization Sequence Completed" in d:
                self.after(0, lambda: self._set_status(True, "ovpn"))
                self.after(2000, self._check_ip)
            elif "AUTH_FAILED" in d:
                self.after(0, lambda: self.log("ERR: Sertifika hatasi!"))
                self.after(0, lambda: self._set_status(False))
                break
            elif "SIGTERM" in d:
                self.after(0, lambda: self._set_status(False))
    ...
    def _disconnect(self):
        ...
        time.sleep(1)
        self.after(0, lambda: self.log("SUCCESS: Baglanti kesildi"))
        self.after(0, lambda: self._set_status(False))
